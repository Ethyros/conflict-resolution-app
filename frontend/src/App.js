...
import { Scale, Handshake, FileText, X, MessageCircle, AlertCircle, Heart, ChevronDown, ChevronUp, Target, Ear } from 'lucide-react';
...
const NVCReferenceList = ({ showNeeds = true }) => {
  ...
  const needs = {
    "Connection": ["acceptance", "appreciation", "belonging", "cooperation", "communication",
    ...
    "Physical Well-being": ["air", "food", "movement/exercise", "rest/sleep", "safety",
      "shelter", "touch", "water"],
    ...
    "Meaning": ["awareness", "celebration", "challenge", "clarity", "competence",
  ...
  const feelings = {
    "Happy": ["amazed", "confident", "delighted", "eager", "energetic", "excited",
      "grateful", "inspired", "joyful", "optimistic", "proud", "relieved",
      "satisfied", "thankful", "trustful"],
    "Peaceful": ["calm", "clear", "comfortable", "content", "relaxed", "rested",
      "secure", "serene", "tranquil"],
    "Afraid": ["alarmed", "anxious", "concerned", "confused", "disturbed", "nervous",
      "scared", "shocked", "stressed", "surprised", "troubled", "uncomfortable",
      "worried"],
    "Angry": ["aggravated", "annoyed", "bitter", "frustrated", "impatient", "irritated",
      "resentful", "upset"],
    "Sad": ["discouraged", "disappointed", "distant", "exhausted", "hopeless",
      "lonely", "overwhelmed", "tired", "vulnerable"],
    "Pain": ["agony", "anguished", "bereaved", "depressed", "despair", "grief",
        ...
        <div className="mt-2 p-4 bg-white rounded-lg border shadow-sm text-sm">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
            {Object.entries(data).map(([category, items]) => (
              <div key={category} className="space-y-1 p-3 bg-gray-50 rounded-lg">
                <h5 className="font-medium text-gray-700">{category}</h5>
                <ul className="list-none text-gray-600 grid grid-cols-2 sm:block gap-1">
                  {items.map((item) => (
...
const FrameworkSelector = ({ onSelect }) => {
  ...
  const handleSelect = (id) => {
    console.log('Framework selected:', id);
    setSelected(id);
    onSelect(id);
  };
      ...
      {frameworks.map((framework) => (
        ...
        >
          <div className="flex items-center space-x-2 sm:space-x-3">
            <div className={selected === framework.id ? 'text-blue-500' : 'text-gray-500'}>
              {framework.icon}
            </div>
            <div>
...
function App() {
  ...
  return (
    <div>
      <div className="min-h-screen bg-gradient-to-br from-blue-100 to-purple-100 flex items-center justify-center p-2 sm:p-4">
        <div className="bg-white shadow-2xl rounded-2xl sm:rounded-3xl w-full max-w-md p-4 sm:p-8 text-center">
          ...
          <div className="space-y-6">
            <div>
              ...
              >
                ...
                <span>Conflict Resolution Analysis</span>
              </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center overflow-y-auto p-0 sm:p-4">
          <div className="bg-white rounded-none sm:rounded-lg w-full max-w-2xl sm:my-8 mx-auto relative min-h-screen sm:min-h-0">
            <button
            ...
            <div className="p-6">
              ...
              {showError && (
                <div className="mb-4 p-3 bg-red-50 text-red-700 rounded-lg flex items-start">
                  <AlertCircle className="mr-2 mt-1 flex-shrink-0" size={16} />
                  <div>
                    <p className="font-medium">Having trouble?</p>
                    <p className="text-sm">If you're experiencing issues, try refreshing the page or coming back later.</p>
                  </div>
                </div>
              )}
              ...
              <div className="mt-2">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full h-40 sm:h-32 p-3 border rounded-lg resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-base"
                  />

                  <button
                    onClick={handleTextSubmit}
                    disabled={isProcessing || !textInput.trim()}
                    className="w-full sm:w-auto mt-3 px-4 py-3 sm:py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:opacity-50"
                  >
                ...
                <ProcessingStates
                  ...
                  processingMessage={mode === 'nvc' ? "Translating to NVC..." : "Analyzing your message..."}
                />
                ...
                <div className="mt-4 space-y-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg text-left border">
                    <h3 className="font-medium text-blue-700 text-lg mb-3">
                      {mode === 'nvc' ? 'NVC Translation & Insights:' : 'Conflict Resolution Analysis:'}
                    </h3>
                    <div className="prose prose-sm max-w-none">
                      {recordedText && recordedText.split('\n').map((line, i) => (
                        <p key={i} className="my-2">{line}</p>
                      ))}
                      {!recordedText && 'No analysis yet...'}
                    </div>
                  </div>
                </div>
              </div>
